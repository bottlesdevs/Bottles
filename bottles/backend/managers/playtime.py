...
class ProcessSessionTracker:
    ...
    def get_weekly_playtime(
        ...
        """
        if not self.enabled:
            logging.debug("Playtime tracking disabled; get_weekly_playtime returns zeros")
            return [0] * 7
        ...
        try:
            with self._lock:
                ...
                """, (bottle_id, program_id, str(week_offset * 7), str((week_offset + 1) * 7)))
    ...
    def get_daily_playtime(
        ...
        """
        if not self.enabled:
            logging.debug("Playtime tracking disabled; get_daily_playtime returns zeros")
            return [0] * 24
            ...
            with self._lock:
                ...
                for row in cur.fetchall():
                    ...
                    while current_timestamp < ended_at:
                        ...
                        next_hour = datetime(dt.year, dt.month, dt.day, dt.hour, 59, 59).timestamp() + 1
                        
                        # Calculate time spent in this hour (in seconds)
                        time_in_hour_seconds = min(ended_at, next_hour) - current_timestamp
    ...
    def get_monthly_playtime(
        ...
        """
        if not self.enabled:
            logging.debug("Playtime tracking disabled; get_monthly_playtime returns zeros")
            return [0] * 12
        ...
        try:
            with self._lock:
                ...
                for row in cur.fetchall():
                    ...
                    total_seconds = int(row[1] or 0)
                    result[month - 1] = total_seconds // 60  # Convert to minutes, adjust to 0-11 indexing
    ...
    def get_weekly_session_count(self, bottle_id: str, program_id: str, week_offset: int = 0) -> int:
        """
    ...
    def get_daily_session_count(self, bottle_id: str, program_id: str, date_str: str) -> int:
        """
    ...
    def get_yearly_session_count(self, bottle_id: str, program_id: str, year: int) -> int:
        """
