...
@Gtk.Template(resource_path='/com/usebottles/bottles/nvapi-entry.ui')
class NvapiEntry(Handy.ActionRow):
    __gtype_name__ = 'NvapiEntry'
    ...
    def __init__(self, window, nvapi, **kwargs):
        ...
        self.nvapi_name = nvapi[0]
        self.spinner = Gtk.Spinner()

        # populate widgets
        self.set_title(self.nvapi_name)

        if nvapi[1].get("Installed"):
        ...
        # connect signals
        self.btn_download.connect('pressed', self.download_nvapi)
        self.btn_err.connect('pressed', self.download_nvapi)
        self.btn_browse.connect('pressed', self.run_browse)

    '''Install nvapi'''

    def download_nvapi(self, widget):
    ...
    def run_browse(self, widget):
        self.btn_download.set_visible(False)
        Runner().open_filemanager(path_type="nvapi", nvapi=self.nvapi_name)
